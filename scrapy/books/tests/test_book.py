...
class BookSpiderTest(unittest.TestCase):
    ...
    def test_parse_scrapes_correct_bool_information(self):
        """Test if the spider scrapes the correct information for each book."""
        results_generator = self.spider.parse(self.response)

        # Book 1
        book1 = next(results_generator)
        self.assertEqual(book1["url"], "catalogue/a-light-in-the-attic_1000/index.html")
        self.assertEqual(book1["title"], "A Light in the Attic")
        self.assertEqual(book1["price"], "£51.77")

        # Book 2
        book2 = next(results_generator)
        self.assertEqual(book2["url"], "catalogue/tipping-the-velvet_999/index.html")
        self.assertEqual(book2["title"], "Tipping the Velvet")
        self.assertEqual(book2["price"], "£53.74")
